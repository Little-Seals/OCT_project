...
void setup_binsizef(int type, int dim, cufinufft_opts *opts)
{
	switch(dim)
	{
		case 1:
...
int cufinufftf_default_opts(int type, int dim, cufinufft_opts* opts)
	...
	{
		int ier;
		opts->upsampfac = (float)2.0;

		/* following options are for gpu: 以下选项适用于 gpu */
		opts->gpu_nstreams = 0;
...
*/
int cufinufftf_makeplan(int type, int dim, int *nmodes, int iflag, int ntransf, float tol, int maxbatchsize, cufinufftf_plan*d_plan_ptr, cufinufft_opts *opts)
{
	...
	cufinufftf_plan d_plan = new cufinufftf_plan_s;
	*d_plan_ptr = d_plan;
	...
	if (opts==NULL){    // use default opts
		ier = cufinufftf_default_opts(type, dim, &(d_plan->opts));
		if (ier != 0){
	...
	switch(d_plan->dim)
	{
		case 1:
		{
			int n[] = {nf1};
			int inembed[] = {nf1};
			cufftPlanMany(&fftplan,1,n,inembed,1,inembed[0], inembed,1,inembed[0], CUFFT_C2C,maxbatchsize);
		}
		break;
		case 2:
		{
			int n[] = {nf2, nf1};
			int inembed[] = {nf2, nf1};
			cufftPlanMany(&fftplan,2,n,inembed,1,inembed[0]*inembed[1],inembed,1,inembed[0]*inembed[1], CUFFT_C2C,maxbatchsize);
		}
		break;
		case 3:
		{
			...
			cufftPlanMany(&fftplan,3,n,inembed,1,inembed[0]*inembed[1]* inembed[2],inembed,1,inembed[0]*inembed[1]*inembed[2], CUFFT_C2C,maxbatchsize);
		}
		break;
	}
	...
	checkCudaErrors(cudaMalloc(&d_f, dim*MAX_NQUAD*sizeof(float)));
	checkCudaErrors(cudaMemcpy(d_a,a,dim*MAX_NQUAD*sizeof(cuDoubleComplex),cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_f,f,dim*MAX_NQUAD*sizeof(float),cudaMemcpyHostToDevice));
	ier = CUFSERIESKERNELCOMPUTE(d_plan->dim, nf1, nf2, nf3, d_f, d_a, d_plan->fwkerhalf1, d_plan->fwkerhalf2, d_plan->fwkerhalf3, d_plan->spopts.nspread);
#ifdef TIME
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&milliseconds, start, stop);
...
*/
int cufinufftf_setpts(int M, float* d_kx, float* d_ky, float* d_kz, int N, float *d_s, float *d_t, float *d_u, cufinufftf_plan d_plan)
{
    // Mult-GPU support: set the CUDA Device ID:
    int orig_gpu_device_id;
    cudaGetDevice(& orig_gpu_device_id);
...
*/
int cufinufftf_execute(CUCPX* d_c, CUCPX* d_fk, cufinufftf_plan d_plan)
{
    // Mult-GPU support: set the CUDA Device ID:
    //int orig_gpu_device_id;
    //cudaGetDevice(& orig_gpu_device_id);
	...
	switch(d_plan->dim)
	{
		...
		{
			if(type == 1)
				ier = cufinufftf1d1_exec(d_c, d_fk, d_plan);
			if(type == 2)
				ier = cufinufftf1d2_exec(d_c, d_fk, d_plan);
			if(type == 3){
				cerr<<"Not Implemented yet"<<endl;
				ier = 1;
			}
...
int cufinufftf_destroy(cufinufftf_plan d_plan)
{
	...
	// Can't destroy a Null pointer. Multi-GPU support: reset the device ID
	if(!d_plan) {
		cudaSetDevice(orig_gpu_device_id);
		return 1;
    }
