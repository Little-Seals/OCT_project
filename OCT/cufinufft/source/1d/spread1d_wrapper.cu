...
int CUFINUFFT_SPREAD1D(int nf1, CUCPX* d_fw, int M, FLT *d_kx, CUCPX *d_c,  CUFINUFFT_PLAN d_plan)
/*
...
int CUSPREAD1D_NUPTSDRIVEN(int nf1, int M, CUFINUFFT_PLAN d_plan, int blksize)
{
	...
	if(d_plan->opts.gpu_kerevalmeth){
		for(int t=0; t<blksize; t++){
			Spread_1d_NUptsdriven_Horner<<<blocks, threadsPerBlock>>>(d_kx, d_c+t*M, d_fw+t*nf1, M, ns, nf1, sigma, d_idxnupts, pirange);
		}
	}else{
		for(int t=0; t<blksize; t++){
			Spread_1d_NUptsdriven<<<blocks, threadsPerBlock>>>(d_kx, d_c+t*M, d_fw+t*nf1, M, ns, nf1, es_c, es_beta, d_idxnupts, pirange);
		}
	}
