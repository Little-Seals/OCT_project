...
// to N/2-1)
__global__ void Deconvolve_1d(int ms, int nf1, CUCPX* fw, CUCPX *fk, FLT *fwkerhalf1)
{
...
__global__ void Deconvolve_2d(int ms, int mt, int nf1, int nf2, CUCPX* fw, CUCPX *fk, FLT *fwkerhalf1, FLT *fwkerhalf2)
{
...
__global__ void Deconvolve_3d(int ms, int mt, int mu, int nf1, int nf2, int nf3, CUCPX* fw, CUCPX *fk, FLT *fwkerhalf1, FLT *fwkerhalf2, FLT *fwkerhalf3)
{
...
__global__ void Amplify_1d(int ms, int nf1, CUCPX* fw, CUCPX *fk, FLT *fwkerhalf1)
{
	for(int i=blockDim.x*blockIdx.x+threadIdx.x; i<ms; i+=blockDim.x*gridDim.x){
		int w1 = i-ms/2 >= 0 ? i-ms/2 : nf1+i-ms/2;
...
__global__ void Amplify_2d(int ms, int mt, int nf1, int nf2, CUCPX* fw, CUCPX *fk, FLT *fwkerhalf1, FLT *fwkerhalf2)
{
	for(int i=blockDim.x*blockIdx.x+threadIdx.x; i<ms*mt; i+=blockDim.x*gridDim.x){
		int k1 = i % ms;
		int k2 = i / ms;
...
__global__ void Amplify_3d(int ms, int mt, int mu, int nf1, int nf2, int nf3, CUCPX* fw, CUCPX *fk, FLT *fwkerhalf1, FLT *fwkerhalf2, FLT *fwkerhalf3)
{
...
*/
{
	...
	if(d_plan->spopts.spread_direction == 1){
		for(int t=0; t<blksize; t++){
			Deconvolve_1d << <(nmodes + 256 - 1) / 256, 256 >> > (ms, nf1, d_plan->fw + t * nf1, d_plan->fk + t * nmodes, d_plan->fwkerhalf1);
		}
	}else{
		checkCudaErrors(cudaMemset(d_plan->fw,0,maxbatchsize*nf1*sizeof(CUCPX)));
		for(int t=0; t<blksize; t++){
			Amplify_1d<<<(nmodes+256-1)/256, 256>>>(ms, nf1, d_plan->fw+t*nf1, d_plan->fk+t*nmodes, d_plan->fwkerhalf1);
#ifdef DEBUG
			CPX* h_fw;
...
int CUDECONVOLVE2D(CUFINUFFT_PLAN d_plan, int blksize) /* wrapper for deconvolution & amplication in 2D.Melody Shih 07/25/19 */
{
	int ms=d_plan->ms;
	int mt=d_plan->mt;
	int nf1=d_plan->nf1;
...
int CUDECONVOLVE3D(CUFINUFFT_PLAN d_plan, int blksize) /* wrapper for deconvolution & amplication in 3D. Melody Shih 07/25/19 */
{
	int ms=d_plan->ms;
	int mt=d_plan->mt;
	int mu=d_plan->mu;
